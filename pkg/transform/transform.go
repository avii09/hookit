...
type TransformationRules struct {
	...
	AddFields   []AddFieldRule    `yaml:"add_fields"`
}
...
func ApplyTransformations(data []map[string]string, rules TransformationRules) []map[string]string {
	...
	// Add New Fields
	data = addFields(data, rules.AddFields)
...
// Helper function to add new fields
func addFields(data []map[string]string, fields []AddFieldRule) []map[string]string {
	for _, row := range data {
		for _, field := range fields {
			if field.Value == "CURRENT_DATE" {
				row[field.Column] = time.Now().Format("2006-01-02")
			} else {
